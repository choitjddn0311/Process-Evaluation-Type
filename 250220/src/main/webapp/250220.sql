...
//회원매출 조회 - 회원번호,회원성명,매출횟수,매출수량,매출액
select m.custno, m.custname, count(n.custno), sum(n.amount), sum(n.price)
from member_02 m left join money_02 n
on m.custno = n.custno
group by m.custno,m.custname
order by m.custno;

//홈쇼핑 회원 등록 - 회원번호,회원성명,회원전화,회원주소,가입일자,고객등급,도시코드
